...
class CsHomeTab extends Module
{
	...
	public function init_data()
	{
		...
		(2, "'.$id_en.'", 1, "Computers"),
		(2, "'.$id_fr.'", 1, "Computers"),
		(3, "'.$id_en.'", 1, "Viewed Products"),
		(3, "'.$id_fr.'", 1, "Visualize Produte"),
		;
		')
		)
			return false;
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('saveTab'))
		{
			...
			parent::_clearCache('cshometab.tpl');
	...
	private function initForm()
	{
		...
		$helper->back_url = $currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules');
		...
		$helper->toolbar_btn =  array(
			...
			array(
				'desc' => $this->l('Save'),
				'href' => $currentIndex.'&configure='.$this->name.'&save'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'),
			),
			'back' =>
			array(
				'href' => $currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'),
	...
	private function getTabsDisplayJS($nb = 10,$id_shop, $tab='')
	{
		$tabsFull = array();
		...
		{
			$tabsFull[] = (object)array('id_tab' => $row['id_tab']);
		}
		return $tabsFull;
	}

	private function getTabsDisplay($nb = 10,$id_shop, $tab='')
	{
		$tabs = array();
	 	$results = Db::getInstance()->ExecuteS(
					'SELECT ts.`id_tab` FROM `'._DB_PREFIX_.'cshometab_shop` ts
		...
		foreach ($results as $row)
		{
			...
			if($temp->product_list == true){
                if($tab == 'ini'){
                    if ($row['id_tab'] == '19' or $row['id_tab'] == '20'){
                        $tabs[] = $temp;
                    }
                }elseif($tab == 'middle'){
                    if ($row['id_tab'] == '21'){
                    ...
                    if ($row['id_tab'] == '22' or $row['id_tab'] == '17' or $row['id_tab'] == '1' or $row['id_tab'] == '10'){
                        $tabs[] = $temp;
                    }
                }
            }
	...
	{
		global $smarty;
		// bug do css para novidades
		if (($smarty->tpl_vars['page_name']->value == 'module-cshometab-newtools') || ($smarty->tpl_vars['page_name']->value == 'module-cshometab-view'))
		{
		...
		if ($smarty->tpl_vars['page_name']->value == 'index')
		{
			...
			$tema_atual = $this->context->theme->name;
			$novo_padrao = Configuration::get('NOVO_PADRAO');

			if($tema_atual == $novo_padrao){
				$this->context->controller->addCss($smarty->tpl_vars['tpl_dir']->value.'css/alexia_core_css.css');
			}else{
			...
			{
				$this->context->controller->addJs($this->_path.'js/jquery-ui-tabs.min.js');
			}
		}
	}

	public static function retornamaisvendidos($id_product, $quantity = 100)
	{
	...
	WHERE ps.id_product = '.(int)$id_product.' AND ps.`quantity` >= '.$quantity.' limit 100');
	return $result;
	...
	public function hookDisplayHome()
	{
		...
		$id_shop = $this->context->shop->id;
        //ini
		if (!$this->isCached('cshometab.tpl',$this->getCacheId('cshometab-ini')))
		{
			if (file_exists(dirname(__FILE__).'/'.'option_'.$id_shop.'.xml')){
                $option = simplexml_load_file(dirname(__FILE__).'/'.'option_'.$id_shop.'.xml');
            }else{
            ...
            $tabs = $this->getTabsDisplay($option->numProduct,$id_shop, 'ini');
			$smarty->assign(array(
				'tabs' => $tabs,
				'option' => $option,
				'theme'  => Configuration::get('NOVO_PADRAO')
			));
		}
        $tpl .= $this->display(__FILE__, 'cshometab.tpl',$this->getCacheId('cshometab-ini'));
        unset($tabs, $option);

				// blog
			if(Configuration::get('GET_BLOG_PROJECTS_HOME') == 1){
					include_once(dirname(__FILE__).'/../../blog/blog-external.php');
					$qtd_posts = 9;
					$posts = getLastPosts($qtd_posts);//faz a consulta no codigo do mauro
					// var_dump($posts);
					$smarty->assign(array(
						'posts' => $posts
					));
					$tpl .= $this->display(__FILE__, 'cshometab-blogpost.tpl');
					unset($posts);
        ...
        // middle
        	if (file_exists(dirname(__FILE__).'/'.'option_'.$id_shop.'.xml'))
				...
				$option = simplexml_load_file(dirname(__FILE__).'/'.'option_'.Configuration::get('PS_SHOP_DEFAULT').'.xml');
			$tabs = $this->getTabsDisplay($option->numProduct,$id_shop, 'middle');
			$smarty->assign(array(
				'tabs' => $tabs,
				'option' => $option
			));
        $tpl .= $this->display(__FILE__, 'cshometab.tpl');

        unset($tabs, $option);
        //end
		if (!$this->isCached('cshometab.tpl',$this->getCacheId('cshometab-end')))
		{
			if (file_exists(dirname(__FILE__).'/'.'option_'.$id_shop.'.xml')){
                $option = simplexml_load_file(dirname(__FILE__).'/'.'option_'.$id_shop.'.xml');
            }else{
            ...
            $tabs = $this->getTabsDisplay($option->numProduct,$id_shop, 'end');
        ...
        $tpl .= $this->display(__FILE__, 'cshometab.tpl',$this->getCacheId('cshometab-end'));

		$tabsFull = $this->getTabsDisplayJS($option->numProduct,$id_shop);
		$smarty->assign(array(
			'tabsFull' => (object)$tabsFull
		));
    	$tpl .= $this->display(__FILE__, 'cshometab-js.tpl');
        return $tpl;
